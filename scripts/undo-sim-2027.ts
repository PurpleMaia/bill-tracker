/**
 * Removes everything created by scripts/seed-sim-2027.ts.
 *
 * Deletion is keyed strictly on the bill_url prefix used by that seed script,
 * so real bills and every other dummy set are never touched. All rows
 * referencing the seeded bills are removed first, then the bills themselves —
 * including any tracking / testimony rows testers created against them.
 *
 * Run: npx tsx scripts/undo-sim-2027.ts
 */
...
async function main() {
  ...
  if (bills.length === 0) {
    console.log('No seeded sim-2027 bills found — nothing to undo.');
    await db.destroy();
    return;
  }
  ...
  console.log(`Removed ${bills.length} simulated 2027 bills and all their related rows:`);
