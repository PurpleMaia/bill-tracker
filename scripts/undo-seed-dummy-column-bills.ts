/**
 * Removes everything created by scripts/seed-dummy-column-bills.ts.
 *
 * Deletion is keyed strictly on the bill_url prefix used by that seed script,
 * so real bills and the pre-existing dummy bills (HB9901–SB9904, which use a
 * different URL path) are never touched. All rows referencing the seeded
 * bills are removed first, then the bills themselves.
 *
 * Run: npx tsx scripts/undo-seed-dummy-column-bills.ts
 */
...
async function main() {
  ...
  if (bills.length === 0) {
    console.log('No seeded dummy column bills found — nothing to undo.');
    await db.destroy();
    return;
  }
  ...
  console.log(`Removed ${bills.length} seeded dummy bills and all their related rows:`);
